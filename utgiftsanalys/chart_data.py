...
def monthly_actuals(conn, account: str | None = None) -> list[dict]:
...
def monthly_with_predictions(conn, account: str | None = None) -> list[dict]:
    ...
    for rec in monthly_actuals(conn, account=account):
        month = rec["month"]
        predicted = sum(l.predicted_amount for l in predict_month(exp_patterns, month))
        rows.append({
            "month": month,
